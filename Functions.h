...
constexpr unsigned int Edinicka(const std::string& str) {
    return 1;
}
...
unsigned int avgascii(const std::string& str) {
    unsigned int avg = 0;
...
unsigned int wow(const std::string& str) {
    if (str.length() == 0)
        return 0;

    unsigned int avg = 0;
    for (char i : str) {
        avg = avg ^ (unsigned int) i;
...
unsigned int CRC32 (const std::string& str) {
    ...
    for (char i : str)
        crc = (crc << 8) ^ crc32_table[((crc >> 24) ^ (unsigned int) i) & 255];

    return crc;
}
#endif //HASH_TABLE_FUNCTIONS_H
